	...
	public static void editarCarro() {
		...
		if (opcao == 1) {
			...
			for (int i = 0; i < carroLista.size(); i++) {
				...
				System.out.println((i + 1) + " - Montadora: "
						+ (carroLista.get(i)).getMontadora());
				System.out.println("    Modelo: "
						+ (carroLista.get(i)).getModelo());
				System.out
						.println("    Tipo: " + (carroLista.get(i)).getTipo());
				System.out.println("    Cor: " + (carroLista.get(i)).getCor());
				System.out.println("    Motor: "
						+ (carroLista.get(i)).getMotor());
				System.out.println("    Câmbio: "
						+ (carroLista.get(i)).getCambio());
				System.out.println("    Preço: "
						+ formatoMoeda.format((carroLista.get(i)).getPreco()));
			...
			if (opcao == 1) {
				...
				// Abre a conexão com o banco, executa a query e verifica
				// exceções;
				try {

					String sql = "UPDATE carro" + " set montadora='"
							+ carroLista.get(0).getMontadora() + "'"
							+ ", modelo='" + carroLista.get(0).getModelo()
							+ "'" + ", tipo='" + carroLista.get(0).getTipo()
							+ "'" + ", cor='" + carroLista.get(0).getCor()
							+ "'" + ", motor='" + carroLista.get(0).getMotor()
							+ "'" + ", cambio='"
							+ carroLista.get(0).getCambio() + "'" + ", preco="
							+ carroLista.get(0).getPreco() + " WHERE chassi='"
							+ chassi + "'";
					...
					for (int i = 0; i < carroLista.size(); i++) {
						System.out
								.println("Veículo atualizado com os seguintes dados: ");
						System.out.println((i + 1) + " - Modelo: "
								+ carroLista.get(0).getModelo());
						System.out.println("    Montadora: "
								+ carroLista.get(0).getMontadora());
						System.out.println("    Tipo: "
								+ carroLista.get(0).getTipo());
						System.out.println("    Cor: "
								+ carroLista.get(0).getCor());
						System.out.println("    Câmbio: "
								+ carroLista.get(0).getCambio());
						System.out.println("    Motor: "
								+ carroLista.get(0).getMotor());
						System.out.println("    Preço: "
								+ formatoMoeda.format(carroLista.get(0)
										.getPreco()));
						System.out
								.println("-------------------------------------------------");
					}
			...
			if (opcao != 1 && opcao != 2) {
				System.out
						.println("Opção Inválida, retornando ao menu principal.");
			}
		}
	}
	...
	public static void editarMoto() {
		...
		if (chassi == null) {
			...
			if (opcao == 1) {
				...
				// Abre a conexão com o banco, executa a query e verifica por
				// execeções;
				try {

					String sql = "UPDATE moto" + " set montadora='"
							+ moto.getMontadora() + "'" + ", modelo='"
							+ moto.getModelo() + "'" + ", tipo='"
							+ moto.getTipo() + "'" + ", cor='" + moto.getCor()
							+ "'" + ", cilindrada='" + moto.getCilindrada()
							+ "'" + ", capacidade_tanque='"
							+ moto.getCapacidadeTanque() + "'" + ", preco="
							+ moto.getPreco() + " WHERE chassi='" + chassi
							+ "'";
					...
					for (int i = 0; i < motoLista.size(); i++) {
						...
						System.out.println("    Preço: "
								+ formatoMoeda.format(moto.getPreco()));
						System.out
								.println("-------------------------------------------------");
					}
